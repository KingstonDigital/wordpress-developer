...
### 8. Static Block Markup Generation (The `<!-- wp:` format)

When generating raw block markup for pasting directly into the WordPress Code Editor:

1.  **Strict Validation**: Follow the rules in `references/gutenberg-serialization.md` to prevent "Block Recovery" errors.
2.  **No Internal Comments**: Never place HTML comments inside a native block's wrapper `<div>`.
3.  **Attribute Matching**: Ensure JSON attributes in comments exactly match HTML classes and styles.
4.  **Layout Precision**: Explicitly define `contentSize` and layout types in `core/group` blocks.
...
-   `references/gutenberg-serialization.md`: **NEW** - Crucial rules for valid, high-precision static block markup (HTML + comments).
